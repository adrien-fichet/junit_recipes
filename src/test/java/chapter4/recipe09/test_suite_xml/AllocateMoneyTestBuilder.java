...
public class AllocateMoneyTestBuilder {
    ...
    private List makeTests() throws Exception {
        ...
        NodeList testNodes = XPathAPI.selectNodeList(document, "/tests/test");
    ...
    private AllocateMoneyTest makeAllocateMoneyTest(Node eachTestNode) throws TransformerException, ParseException {
        Money amountToSplit = parseAsMoney(eachTestNode, "input/amount-to-split");
        int nWays = parseAsInt(eachTestNode, "input/number-of-ways");
        NodeList expectedCutNodes = XPathAPI.selectNodeList(eachTestNode, "expected-result/cut");
    ...
    private Document makeDocument(String documentFileName) throws FactoryConfigurationError,
            ParserConfigurationException, SAXException, IOException {
        DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
        DocumentBuilder builder = factory.newDocumentBuilder();
        InputStream testDataAsStream = AllocateMoneyTestBuilder.class.getResourceAsStream(documentFileName);
        Document document = builder.parse(testDataAsStream);
        return document;
    }

    private Map parseExpectedCuts(NodeList expectedCutNodes) throws TransformerException, ParseException {
        Map expectedCuts = new HashMap();

        for (int i = 0; i < expectedCutNodes.getLength(); i++) {
            Node eachCutNode = expectedCutNodes.item(i);
            Money cutAmount = parseAsMoney(eachCutNode, "amount");
    ...
    private static int parseAsInt(Node fromNode, String xpathToInt) throws TransformerException {
        String intAsString = getNodeText(fromNode, xpathToInt);
        return Integer.parseInt(intAsString);
    }

    private static String getNodeText(Node fromNode, String xpath) throws TransformerException {
        Text text = (Text) XPathAPI.selectSingleNode(fromNode, xpath).getFirstChild();
        return text.getData();
    }

    private static Money parseAsMoney(Node fromNode, String xpathToMoneyObject) throws TransformerException,
            ParseException {
